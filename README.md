...
To control your device locally, you need credentials generated by the official Philips HomeID app. There are two methods depending on your device model.

### Method 1: SQLite Database (HTTPS devices — most Air Purifiers)
...
4. **Extract credentials**
   ...
   - Find `client_id` and `client_secret` columns
   - Enter these values during integration setup

</details>

### Method 2: SharedPreferences (HTTP devices — Air Fryers like HD9285)

Some newer devices (especially HTTP-based air fryers) store credentials in Android's EncryptedSharedPreferences instead of the SQLite database. If `network_node.db` is empty, use this method.
...
3. **Extract credentials from SharedPreferences**
   - Using a root file manager or adb shell, navigate to:
     ```
     /data/data/com.versuni.homeid/shared_prefs/
     ```
   - Look for the file `ONE_KA_PREFERENCES_SECURE.xml` (or similar preference files)
   - Find the `client_id`, `client_secret`, and `encryption_key` values
   - Enter these values during integration setup
   - Note: `encryption_key` is only needed for HTTP devices. If left empty, the integration will try to fetch it automatically from the device.
...
### Empty Database
If `network_node.db` is empty in the SQLite editor, your device stores credentials in EncryptedSharedPreferences instead. See [Method 2](#method-2-sharedpreferences-http-devices--air-fryers-like-hd9285) in the credential extraction section.
