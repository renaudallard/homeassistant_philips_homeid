...
The app stores credentials in an **unencrypted** SQLite database. This is the easiest method.
...
### Method 2: SharedPreferences (if database is empty)

Some devices don't write credentials to the SQLite database. In that case, credentials may be stored in SharedPreferences files.
...
3. **Extract credentials from SharedPreferences**
   - Using a root file manager or adb shell, navigate to:
     ```
     /data/data/com.philips.ka.oneka.app/shared_prefs/
     ```
