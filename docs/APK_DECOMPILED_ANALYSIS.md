...
```java
// FusionAuthenticationInitUseCaseImpl.e() (line 64-76):
suspend fun e() {
    if (!philipsUser.m()) throw "Not logged in"  // m() = isLoggedInToNutriU
    if (d()) {                                    // d() = isTokenExpired
        diDaBridge.b().await()                   // b() = RefreshDiDaSessionUseCase
        // Refreshes OIDC tokens via AppAuth:
        //   POST {oidcTokenEndpoint}
        //   grant_type=refresh_token
        //   client_id=-u6aTznrxp9_9e_0a57CpvEG
        //   refresh_token={stored_refresh_token}
        // AuthState updated with new access_token, id_token, expiry
    }
}

// d() = isTokenExpired (line 48-62):
fun d(): Boolean {
    val expiresIn = philipsUser.X()  // diAccessExpiresIn (milliseconds)
    if (expiresIn == null || expiresIn == 0L) return true  // no expiry = refresh
    return expiresIn < System.currentTimeMillis()          // expired = refresh
}
```

The refresh uses the same `client_id` and OIDC token endpoint as our
`cloud_api.refresh_tokens()` method. The refreshed token remains a
Gigya OIDC access_token with `aud=-u6aTznrxp9_9e_0a57CpvEG`.

The `authenticationFailureHandler` (line 208-272) also has a
`refreshAccessToken` method that does the same:
`diDaBridge.b().await()` then `philipsUser.l()`.
